...
class AFD_infer(nn.Module):
    def __init__(self, args):
        super(AFD_infer, self).__init__()
    ...
    def forward(self, g_s, g_t,weight):
        ...
        loss_k,att_values = self.attention(g_s, g_t)
        print('att_values',att_values.shape)
        # loss=0
        # for i in range(len(loss_k)):
        #     loss+=loss_k[i]*weight[i]
        # weight=weight.cpu()
        # loss_sum =torch.mul(A, W).mean(1) 
        # print('att_loss',loss.shape)
        return sum(loss_k),att_values
        # return torch.tensor(np.average(loss.cpu(), weights=weight))
...
class Attention(nn.Module):
    ...
    def forward(self, g_s, g_t):
        ...
        # print('bilinear_key',bilinear_key.shape)
        # print('h_hat_s_all',h_hat_s_all.shape)
...
class LinearTransformTeacher(nn.Module):
    ...
    def forward(self, g_t):
        ...
        # print('channel_mean',len(channel_mean))
        # print('spatial_mean',len(spatial_mean))
        # print('spatial_mean0_shape',spatial_mean[0].shape)
